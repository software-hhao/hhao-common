...
public class NumberUtils {
    ...
    public static BigDecimal round(String numberStr, int scale, RoundingMode roundingMode) {
        if (!StringUtils.hasText(numberStr)){
            throw new IllegalArgumentException("number string is blank");
        }
        if (scale < 0) {
            scale = 0;
        }
    ...
    public static Integer[] generateBySet(int begin, int end, int size) {
        ...
        // 加入逻辑判断，确保begin<end并且size不能大于该表示范围
        if ((end - begin) < size) {
            throw new ServerRuntimeException("Size is larger than range between begin and end!");
        }
    ...
    public static boolean isGreater(BigDecimal bigNum1, BigDecimal bigNum2) {
        Assert.notNull(bigNum1,"bigNum1 can't null");
        Assert.notNull(bigNum2,"bigNum2 can't null");
    ...
    public static boolean isGreaterOrEqual(BigDecimal bigNum1, BigDecimal bigNum2) {
        Assert.notNull(bigNum1,"num1 not null");
        Assert.notNull(bigNum2,"num2 not null");
        return bigNum1.compareTo(bigNum2) >= 0;
    }
    ...
    public static BigInteger toBigInteger(String number) {
        return StringUtils.hasText(number) ? new BigInteger(number):BigInteger.ZERO;
    }
